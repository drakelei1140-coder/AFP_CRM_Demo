...
export const EnterpriseListPage = () => {
  ...
  const thirdRegionOptions = useMemo<Record<string, Array<{ value: string; label: string }>>>(() => ({
    HK: [{ value: 'HK-Island', label: '香港岛' }, { value: 'HK-Kowloon', label: '九龙' }, { value: 'HK-NewTerritories', label: '新界' }],
    SG: [{ value: 'SG-Central', label: 'Central Region' }, { value: 'SG-East', label: 'East Region' }, { value: 'SG-West', label: 'West Region' }],
    MO: [{ value: 'MO-MacauPeninsula', label: '澳门半岛' }, { value: 'MO-Taipa', label: '氹仔' }, { value: 'MO-Coloane', label: '路环' }]
  }), []);
  ...
  const filters = useMemo(() => [
      ...
      node: (
        <Space.Compact style={{ width: '100%' }}>
          <Select
            allowClear
            placeholder="二级地区"
            options={[{ value: 'HK', label: '香港' }, { value: 'SG', label: '新加坡' }, { value: 'MO', label: '澳门' }]}
            onChange={(value) => setSecondaryRegion(value)}
          ...
          <Select
            allowClear
            placeholder="三级地区"
            disabled={!secondaryRegion}
            options={secondaryRegion ? thirdRegionOptions[secondaryRegion] : []}
            style={{ width: '50%' }}
          />
        </Space.Compact>
      )
    ...
    { label: '企业审核状态', node: <Select options={[{ value: '待审核' }, { value: '待风控审核' }, { value: '待第三方审核' }, { value: '审核通过' }, { value: '审核不通过' }]} /> },
  ...
  ], [secondaryRegion, thirdRegionOptions]);
