...
export const ShopCreatePage = () => {
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      ...
      <Form form={form} layout="vertical">
        <Card id="keys" title="商铺主键与关联信息区" extra={<Space><Button onClick={copyFromEnterprise}>从企业复制信息</Button><Button onClick={() => setEnterpriseOpen(true)}>选择企业</Button></Space>}>
          <Row gutter={16}>
            ...
            <Col span={8}><Form.Item label="所属企业编号（CID）" name="所属企业编号（CID）"><Input readOnly placeholder="请先选择企业" value={enterprises.find((x) => x.id === form.getFieldValue('enterpriseId'))?.cid} /></Form.Item></Col>
            <Col span={8}><Form.Item label="所属企业名称" name="所属企业名称"><Input readOnly placeholder="请先选择企业" value={enterprises.find((x) => x.id === form.getFieldValue('enterpriseId'))?.name} /></Form.Item></Col>
            ...
            <Col span={8}><Form.Item label="所属地区" name="所属地区"><Input readOnly value={enterprises.find((x) => x.id === form.getFieldValue('enterpriseId'))?.region} /></Form.Item></Col>
            <Col span={8}><Form.Item label="服务通道" name="服务通道"><Input placeholder="保存后生成" /></Form.Item></Col>
            <Col span={8}><Form.Item label="企业选择标识" name="enterpriseId" rules={[{ required: true }]}><Input readOnly placeholder="请使用选择企业弹窗" /></Form.Item></Col>
          </Row>
        </Card>
      ...
      <div className="sticky-bottom-actions" style={{ position: 'sticky', bottom: 0, zIndex: 30 }}><Space><Button onClick={() => setDraftOpen(true)}>从草稿复制</Button><Button onClick={saveDraft}>存成草稿</Button><Button type="primary" onClick={async () => { await form.validateFields(['enterpriseId', '商铺中文名称', '门头名称', '收据显示名称']); message.success('新增商铺已保存，状态为待审核（demo模拟）'); nav('/shops/shop-1'); }}>保存</Button><Button onClick={() => nav(-1)}>取消</Button></Space></div>

      <Modal open={enterpriseOpen} onCancel={() => setEnterpriseOpen(false)} footer={null} title="选择企业" width={960}><Space direction="vertical" style={{ width: '100%' }} size={12}><Row gutter={12}><Col span={12}><Input placeholder="按编号搜索" value={enterpriseKeyword} onChange={(e) => setEnterpriseKeyword(e.target.value)} /></Col><Col span={12}><Input placeholder="按名称模糊搜索" value={enterpriseKeyword} onChange={(e) => setEnterpriseKeyword(e.target.value)} /></Col></Row><Table rowKey="id" dataSource={filteredEnterprises} pagination={false} columns={[{ title: '企业编号', dataIndex: 'cid' }, { title: '企业名称', dataIndex: 'name' }, { title: '地区', dataIndex: 'region' }, { title: '操作', render: (_, r) => <Button type="link" onClick={() => { form.setFieldsValue({ enterpriseId: r.id, '所属企业编号（CID）': r.cid, '所属企业名称': r.name, 所属地区: r.region }); setEnterpriseOpen(false); }}>选择</Button> }]} /></Space></Modal>
