import { Alert, Badge, Button, Card, Descriptions, Form, Input, Modal, Select, Space, Table, Typography, Upload, message } from 'antd';
...
const navSections = [
  ...
  { key: 'afp', title: 'AFP 相关信息展示区' }
] as const;

const groupOrder = ['主键/关联', '企业名称', '企业主体/经营', '企业基础', '企业联系', '企业地址', '注册地址', '主要营业地址', '企业文件', '风控/业务', 'BL/AH'];
...
export const EnterpriseCreatePage = () => {
  ...
  const afpReadonlyFields = useMemo(() => cidFieldConfig.filter((f) => f.source === 'AFP补充'), []);
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%', paddingBottom: 88 }}>
      ...
      <Form form={form} layout="vertical" onValuesChange={refreshSaveState}>
        ...
        <Card id="afp" title="AFP 相关信息展示区">
          <Alert type="info" showIcon message="AFP 相关信息为通道返回字段，新增页仅展示占位，不支持编辑。" style={{ marginBottom: 12 }} />
          <Descriptions bordered column={2} size="small" items={afpReadonlyFields.map((f) => ({ key: f.key, label: f.label, children: '-' }))} />
        </Card>
      </Form>
